...
#argsproduct = event['Product']
#argsversion = event['Version']
argsvendor = ""
argsproduct = "a-cart"
argsversion = "1.0.0"
...
def get_ids_from_cve_page(resp):
    """
    ...
    vendor = argsvendor or title_links[0].text
...
def get_ids_from_searchresults(resp):
    """
    ...
    vendor = argsvendor or row_1.findAll("td")[1].find("a").text
...
resp = request_search(argsvendor,argsproduct,argsversion)
...
if "List of cve security vulnerabilities related to this exact version" not in resp:
    ...
    if "No matches" not in resp:
        ...
        if argsversion in versions_results:
            version_id = versions_results[argsversion]  # list of ids
            version = argsversion
            success(
                "Exact match found in the database (in {} entries, "
                "results will be merged)".format(len(version_id))
            )
            vendor, vendor_id, product_id = get_ids_from_searchresults(resp)
            version_found = True
    ...
    if not version_found:
        ...
        while i >= 0:
            version_search = argsversion[:i] + "%"
            resp = request_search(argsvendor or "", argsproduct, version_search)
            ...
            if "Security Vulnerabilities" in resp:
                ...
                try:
                    if LooseVersion(version) >= LooseVersion(argsversion):
            ...
            # If table with matching versions displayed
                ...
                version = get_closest_superior_version(
                    argsversion, versions_results.keys()
                )
...
#
    ...
    success("Exact match found in the CVE database")

    # Retrieve Vendor & Vendor_id, Product id, Version id from the CVE
    # results page
    vendor, vendor_id, product_id, version, version_id = get_ids_from_cve_page(resp)
